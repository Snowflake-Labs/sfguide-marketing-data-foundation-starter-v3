...
CREATE OR REPLACE VIEW <DB>.SALESFORCE_SAMPLE.SALESFORCE_VW AS 
            ...
            SALESFORCE_PERSON_NAME,STATUS, SPLIT_PART(SPLIT_PART(QUERY_PARAMETERS_UTMS, 'utm_source=', 2),chr(38),0) as SF_UTM_SOURCE,
               SPLIT_PART(SPLIT_PART(SPLIT_PART(QUERY_PARAMETERS_UTMS, 'utm_source=', 2),chr(38)||'utm_medium=',2), chr(38), 1) as SF_UTM_MEDIUM, 
               SPLIT_PART(SPLIT_PART(QUERY_PARAMETERS_UTMS, chr(38)||'utm_campaign=', 2),chr(38),0) as SF_utm_campaign,
               SPLIT_PART(SPLIT_PART(QUERY_PARAMETERS_UTMS, chr(38)||'utm_content=', 2),chr(38),0) as SF_utm_ad_name,
               SPLIT_PART(SPLIT_PART(QUERY_PARAMETERS_UTMS, chr(38)||'ad_id=', 2),chr(38),0) as SF_utm_ad_id
...
select  *, value:value.string_value as QUERY_PARAMETERS_UTMS, SPLIT_PART(SPLIT_PART(QUERY_PARAMETERS_UTMS, 'utm_source=', 2),chr(38),0) as UTM_SOURCE,
               SPLIT_PART(SPLIT_PART(SPLIT_PART(QUERY_PARAMETERS_UTMS, 'utm_source=', 2),chr(38)||'utm_medium=',2), chr(38), 1) as UTM_MEDIUM, 
               SPLIT_PART(SPLIT_PART(QUERY_PARAMETERS_UTMS, chr(38)||'utm_campaign=', 2),chr(38),0) as utm_campaign,
               SPLIT_PART(SPLIT_PART(QUERY_PARAMETERS_UTMS, chr(38)||'utm_content=', 2),chr(38),0) as utm_ad_name,
               SPLIT_PART(SPLIT_PART(QUERY_PARAMETERS_UTMS, chr(38)||'ad_id=', 2),chr(38),0) as utm_ad_id from <DB>.C360_SAMPLE_SCHEMA.CUSTOMER72_GA_LAT_LONG_VW,  LATERAL FLATTEN( INPUT => PARSE_JSON(EVENT_PARAMS_UTMS) );

-- Customer 360 joined view (required for Data Explorer Customer 360 dashboard)
-- Joins Google Analytics data with Salesforce CRM data on UTM parameters
CREATE OR REPLACE VIEW <DB>.<SCHEMA>.C360_CLICKS_CRM_JOINED_VW AS
SELECT 
    SAL02.CAMPAIGN_PARTNER,
    SAL02.CAMPAIGN_PARTNER_OR_EMPLOYEE,
    SAL02.EMPLOYEE,
    SAL02.RESPONSE_COUNTRY,
    CUS01.GEO_COUNTRY,
    CUS01.LAT,
    CUS01.LNG,
    CUS01.USER_PSEUDO_ID,
    CUS01.EVENT_NAME,
    CUS01.USER_LTV_REVENUE,
    CUS01.EVENT_DATE,
    CUS01.EVENT_PARAMS_UTMS,
    CUS01.DEVICE_WEB_INFO_BROWSER,
    CUS01.DEVICE_WEB_INFO_BROWSER_VERSION,
    SAL02.STATUS,
    CUS01.DEVICE_CATEGORY,
    CUS01.UTM_SOURCE,
    CUS01.UTM_MEDIUM,
    CUS01.UTM_CAMPAIGN
FROM <DB>.C360_SAMPLE_SCHEMA.CUSTOMER72_GA_LAT_LONG_UTMS_VW CUS01
INNER JOIN <DB>.C360_SAMPLE_SCHEMA.SALESFORCE_RAW_DATA SAL02
    ON SPLIT_PART(SPLIT_PART(SAL02.QUERY_PARAMETERS_UTMS, 'utm_source=', 2),chr(38),0) = CUS01.UTM_SOURCE 
    AND SPLIT_PART(SPLIT_PART(SPLIT_PART(SAL02.QUERY_PARAMETERS_UTMS, 'utm_source=', 2),chr(38)||'utm_medium=',2), chr(38), 1) = CUS01.UTM_MEDIUM 
    AND SPLIT_PART(SPLIT_PART(SAL02.QUERY_PARAMETERS_UTMS, chr(38)||'utm_campaign=', 2),chr(38),0) = CUS01.UTM_CAMPAIGN 
    AND LENGTH(TRIM(SPLIT_PART(SPLIT_PART(SAL02.QUERY_PARAMETERS_UTMS, 'utm_source=', 2),chr(38),0))) > 0;
